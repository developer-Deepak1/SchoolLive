...
export class ClassesService {
    ...
    createClass(class_: Classes): Observable<Classes | null> {
        ...
        return this.http.post<any>(`${this.baseUrl}/CreateClasses`, payload).pipe(
            map(res => (res && res.data && res.data) ? res.data : null)
        );
    }

    updateClass(class_: Classes): Observable<Classes | null> {
        if (!class_.ClassID) return new Observable(subscriber => { subscriber.next(null); subscriber.complete(); });
        ...
        return this.http.put<any>(`${this.baseUrl}/classes/${class_.ClassID}`, payload).pipe(
            map(res => (res && res.data && res.data) ? res.data : null)
        );
    }

    deleteClass(classId: number): Observable<boolean> {
        return this.http.delete<any>(`${this.baseUrl}/classes/${classId}`).pipe(
