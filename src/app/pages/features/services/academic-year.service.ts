...
export class AcademicYearService {
    ...
    getAcademicYears(): Observable<AcademicYear[]> {
        return this.http.get<AcademicYearResponse>(`${this.baseUrl}/getAcademicYears`).pipe(map((res) => (res && res.success && res.data ? res.data : [])));
    }

    createAcademicYear(year: AcademicYear): Observable<boolean> {
        const payload = this.toApiPayload(year);
        return this.http.post<AcademicYearResponse>(`${this.baseUrl}/CreateAcademicYears`, payload).pipe(map((res) => res && res.success === true));
    }
    ...
    updateAcademicYear(year: AcademicYear): Observable<boolean> {
        ...
        const payload = this.toApiPayload(year);
        return this.http.put<AcademicYearResponse>(`${this.baseUrl}/academicYears/${year.AcademicYearID}`, payload).pipe(map((res) => res && res.success === true));
    }
