    ...
    save() {
        ...
        if (val.StudentID) {
            this.studentsService.updateStudent(val.StudentID, val).subscribe({
                next: (res) => {
                    if (res) {
                        this.msg.add({ severity: 'success', summary: 'Updated', detail: 'Student updated' });
                        this.load();
                    }
                },
            ...
            this.studentsService.createStudent(val).subscribe({
                next: (res) => {
                    if (res) {
                        this.msg.add({ severity: 'success', summary: 'Created', detail: 'Student created' });
                        this.load();
                    }
                },
        ...
        this.confirm.confirm({
            ...
            accept: () => {
                this.studentsService.deleteStudent(stu.StudentID!).subscribe({
                    next: (ok) => {
                        if (ok) {
                            this.msg.add({ severity: 'success', summary: 'Deleted', detail: 'Student deleted' });
                            this.load();
                        }
                    },
