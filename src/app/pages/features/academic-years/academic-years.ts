...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, ReactiveFormsModule, FormsModule, TableModule, ButtonModule, SplitButtonModule, ToastModule, ToolbarModule, InputTextModule, InputIconModule, IconFieldModule, ConfirmDialogModule, DatePickerModule, SelectModule],
...
export class AcademicYears implements OnInit {
    ...
    private enable(name: string) {
        this.yearForm.get(name)?.enable({ emitEvent: false });
    }
    private disable(name: string) {
        this.yearForm.get(name)?.disable({ emitEvent: false });
    }
    ...
    saveYear() {
        ...
        const request$ = val.AcademicYearID ? this.service.updateAcademicYearResponse(val) : this.service.createAcademicYearResponse(val);
    ...
    get isStatusDisabled(): boolean {
        return !!this.yearForm.get('Status')?.disabled;
    }

    get isEndDateDisabled(): boolean {
        return !this.yearForm.get('StartDate')?.value;
    }
