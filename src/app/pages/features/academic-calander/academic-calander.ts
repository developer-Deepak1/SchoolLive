...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, TableModule, CardModule, ButtonModule, SelectModule, DatePickerModule, ToastModule, ConfirmDialogModule, InputTextModule, FormsModule],
...
export class AcademicCalander implements OnInit {
    ...
    private normalizeHoliday(h: any) {
        ...
        if (rawDate) {
            if (typeof rawDate === 'string') {
                ...
                try {
                    date = toLocalYMDIST(new Date(rawDate));
                } catch (e) {
                    date = String(rawDate);
                }
            }
    ...
    addHoliday() {
        ...
        if (this.holidayDate && this.holidayType) {
            ...
            if (this.rangeMode) {
                ...
                if (min && endDate < min) {
                    this.msg.add({ severity: 'warn', summary: 'Invalid Date', detail: `End date must be on or after ${min.toISOString().split('T')[0]}` });
                    return;
                }
                if (max && endDate > max) {
                    this.msg.add({ severity: 'warn', summary: 'Invalid Date', detail: `End date must be on or before ${max.toISOString().split('T')[0]}` });
                    return;
                }
            ...
            accept: () => {
                if (id) {
                    this.calendarService.deleteHoliday(id).subscribe({
                        next: (res) => {
                            this.msg.add({ severity: 'success', summary: 'Deleted', detail: res && res.message ? res.message : 'Holiday deleted' });
                            this.loadHolidays();
                        },
                        error: (err) => {
                            ...
                            const detail = err?.error?.message ?? err?.message ?? 'Server error';
                            this.msg.add({ severity: 'error', summary: 'Delete failed', detail });
                        }
                    });
                } else {
                    this.holidays = this.holidays.filter((x) => x !== h);
    ...
    loadWeeklyReport(start: string, end: string) {
        const ay = this.selectedAcademicYear?.value?.AcademicYearID;
        this.calendarService.getWeeklyReport(start, end, ay).subscribe({
            next: (data: any[]) => {
                console.log('Weekly report', data);
            },
            error: (err) => console.error('Weekly report failed', err)
        });
    }
