...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, TableModule, ButtonModule, InputTextModule, DialogModule, ToastModule, ConfirmDialogModule, FormsModule, ReactiveFormsModule, SelectModule, InputIconModule, IconFieldModule],
        ...
        if (val.EmployeeID) {
            this.employeesService.updateEmployee(val.EmployeeID, val).subscribe({
                next: (res: Employee | null) => {
                    if (res) {
                        this.msg.add({ severity: 'success', summary: 'Updated', detail: 'Employee updated' });
                        this.load();
                    }
                },
            ...
            this.employeesService.createEmployee(val).subscribe({
                next: (res: Employee | null) => {
                    if (res) {
                        this.msg.add({ severity: 'success', summary: 'Created', detail: 'Employee created' });
                        this.load();
                    }
                },
        ...
        this.confirm.confirm({
            ...
            accept: () => {
                this.employeesService.deleteEmployee(emp.EmployeeID!).subscribe({
                    next: (ok: boolean) => {
                        if (ok) {
                            this.msg.add({ severity: 'success', summary: 'Deleted', detail: 'Employee deleted' });
                            this.load();
                        }
                    },
