            ...
            <div class="profile-wrapper">
                ...
                <div class="grid info-grid three">
                    <p-card header="Identity" class="panel">
                        <div class="kv">
                            <div class="row">
                                <span>Username</span><strong>{{ employee()!.Username || '-' }}</strong>
                            </div>
                            <div class="row">
                                <span>Name</span><strong>{{ employee()!.EmployeeName || '-' }}</strong>
                            </div>
                            <div class="row">
                                <span>Gender</span><strong>{{ employee()!.Gender || '-' }}</strong>
                            </div>
                            <div class="row">
                                <span>DOB</span><strong>{{ employee()!.DOB ? (employee()!.DOB | date: 'dd-MMM-yyyy') : '-' }}</strong>
                            </div>
                        </div>
                    </p-card>
                    ...
                    <p-card header="Employment" class="panel">
                        <div class="kv">
                            <div class="row">
                                <span>Role</span><strong>{{ employee()!.RoleName || '-' }}</strong>
                            </div>
                            <div class="row">
                                <span>Joining</span><strong>{{ employee()!.JoiningDate ? (employee()!.JoiningDate | date: 'dd-MMM-yyyy') : '-' }}</strong>
                            </div>
                            <div class="row">
                                <span>Salary</span><strong>{{ employee()!.Salary ? (employee()!.Salary | currency) : '-' }}</strong>
                            </div>
                            <div class="row">
                                <span>Status</span><strong>{{ employee()!.Status || '-' }}</strong>
                            </div>
                        </div>
                    </p-card>
                    ...
                    <p-card header="Contact" class="panel">
                        <div class="kv">
                            <div class="row">
                                <span>Contact</span><strong>{{ employee()!.ContactNumber || '-' }}</strong>
                            </div>
                            <div class="row">
                                <span>Email</span><strong>{{ employee()!.EmailID || '-' }}</strong>
                            </div>
                        </div>
                    </p-card>
                </div>
...
export class EmployeeProfile implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (!id) {
            this.msg.add({ severity: 'error', summary: 'Error', detail: 'Invalid employee id' });
            return;
        }
        ...
        this.employees.getEmployeeMonthlyAttendance().subscribe({
            next: (chart: any) => {
                if (chart?.labels && chart?.datasets && chart.labels.length) {
                    ...
                    if (join) {
                        const joinMon = new Date(join);
                        joinMon.setDate(1);
                        joinMon.setHours(0, 0, 0, 0);
    ...
    edit() {
        ...
        if (e?.EmployeeID) this.router.navigate(['/features/add-employees'], { queryParams: { id: e.EmployeeID } });
    }
